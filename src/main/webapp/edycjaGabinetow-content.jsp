...
<%
	...
	int idDoEdycji = (Integer)request.getAttribute("idDoEdycji");
%>
...
<form id="edycjaForm" action="WyswietlGabinety.do" method="post" class="auxilium" onsubmit="return checkForm(this);" >
...
<!-- 	<label>
	        ...
	        <input type="text" name="numerWyszukiwanegoGabinetu"/>
	        <input type="image" name="wyszukaj" src="icons/ic_action_search.png"
	        	   onclick="form.action='WyszukajGabinet.do';"/>
	        <input class="prawyButton" type="submit" name="powrotDoPanelu" value="Wr&oacute;&cacute; do panelu" 
	        	   onclick="form.action='WrocDoPanelu.do';"/> 
	</label>
	<br/>
	<br/>
	<br/>  -->
	...
	<%
		...
		if (gabinet.getId()== idDoEdycji) {
			out.print("<tr>");
			out.print("<td> <input type='text' name='numerGabinetu' value='"+gabinet.getNumer()+"' pattern='{1,15}' ></td>");
			out.print("<td> <input type='text' name='opisGabinetu' value='"+gabinet.getOpis()+"' pattern='{0,100}'></td>");
			out.print("<td><input type='image' name='anuluj' src='icons/ic_action_undo.png' alt='Anuluj' formaction='WyswietlGabinety.do' title='Anuluj edycj&eogon;'></td>");
			// dla wybranego rekordu wykonaj zapis wyedytowanych wartosci przy pomocy parametru 'edytowaneID'
			out.print("<td><input type='image' name='edytowaneID' src='icons/ic_action_accept.png' alt='Zapisz' formaction='ZapiszGabinet.do'; value='"+gabinet.getId()+"'></td>");
			out.print("</tr>");
		} else {
			out.print("<tr>");
			out.print("<td>"+gabinet.getNumer()+"</td>");
			out.print("<td>"+gabinet.getOpis()+"</td>");
			//out.print("<td><input type='image' name='usuwaneId' src='icons/ic_action_cancel.png' onclick=form.action='UsunGabinet.do'; value='"+gabinet.getId()+"'></td>");
